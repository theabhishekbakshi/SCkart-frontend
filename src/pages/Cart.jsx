                ...
                cart.map((e)=>(
                  <div key={e._id} className="flex flex-col sm:flex-row items-center sm:items-stretch space-y-4 sm:space-y-0 sm:space-x-4 shadow-md rounded-lg p-4 border border-gray-400 ">
                      <img src={e.product.images[0].url} alt={e.product.title}
                        className='w-full sm:w-30 sm:h-30 object-cover rounded-md cursor-pointer'
                        onClick={()=>navigate(`/product/${e.product._id}`)}
                       />
                       <div className="flex-1 text-center sm:text-left">
                        <h2 className="text-lg font-medium">{e.product.title}</h2>
                        <p className="">Price: ₹ {e.product.price}</p>
                       </div>
                       ...
                       <div className="flex items-center justify-center sm:justify-start gap-2">
                        ...
                        <Button variant='ghost' className="text-red-600" onClick={()=>removeFromCart(e._id)}><Trash className='w-6 h-6'/></Button>
                  </div>
                ))}
            </div>
