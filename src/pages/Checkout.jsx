...
const Checkout = () => {
    ...
    const deleteHandler = async (id)=>{
        if(confirm("Are you sure to wanna delete this address!")){
            try {
            ...
            {address && address.length>0? address.map((e)=>(
                <div className="p-4 border rounded-lg shadow-sm" key={e._id}>
                    <h3 className='text-lg font-semibold flex justify-between gap-3'>Address - {e.address}
                        <Button onClick={()=> deleteHandler(e._id)} variant='destructive'><Trash/></Button>
                    </h3>

                    <p className='text-sm my-4'>Phone - {e.phone}</p>
