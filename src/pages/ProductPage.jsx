...
const ProductPage = () => {
  ...
  const submitHandler = async(e) => {
    ...
    try {
      const {data} = await axios.put(`${server}/api/product/${id}`, {title, about, price, stock, category}, {
        headers: {
  ...
  <div>
      ...
      <div className="container mx-auto px-4 py-8 ">
        {
          user && user.role === "admin" && <div className="w-[300px] md:w-[450px] m-auto mb-5">
            ...
            {show && <div className="space-y-4 ">
              ...
              <div>
                <Label>About</Label>
                <Input placeholder="About Product" value={about} onChange={e=>setAbout(e.target.value)} required/>
              </div>
              ...
              <div>
                <Label>Price</Label>
                <Input placeholder="Product Price" value={price} type="number" onChange={e=>setPrice(e.target.value)} required/>
              </div>

              <div>
                <Label>Stock</Label>
                <Input placeholder="Product Stock" value={stock} type="number" onChange={e=>setStock(e.target.value)} required/>
              </div>

              <Button onClick={submitHandler} type="submit" className="w-full" disabled={btnLoading}>{
        ...
        {product && (
          <div className="flex flex-col lg:flex-row items-start gap-14 ">
            <div className="w-[290px] md:w-[650px]">
              ...
              {user && user.role === "admin" && <form onSubmit={handleSubmitImage} className="flex flex-col gap-4"><div><Label>Upload New Images:</Label>
              ...
              <Button type="submit" disabled={btnLoading}>Update Image</Button>
              </form>}
            </div>
    ...
    {relatedProduct?.length>0 && 
      <>
      {
        loading ? <Loading/> : (<div className="mt-12 ">
